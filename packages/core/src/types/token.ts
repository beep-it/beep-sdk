...
export class TokenUtils {
   ...
   * @returns Number of decimal places (e.g., 6 for USDT)
   * 
   * @example
   * ```typescript
   * const decimals = TokenUtils.getTokenDecimals(SupportedToken.USDT);
   * const baseUnits = 1.5 * Math.pow(10, decimals); // Convert 1.5 USDT to base units
   * ```
   */
